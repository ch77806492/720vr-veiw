...
function chooseGuideAsset(action){const assets=roamingConfig.guide&&roamingConfig.guide.assets||{},preferred=assets[action],idle=assets.idle;if(preferred&&preferred.url)return preferred;if(idle&&idle.url)return idle;return Object.values(assets).find(function(asset){return asset&&asset.url})||null}
function guidePlacement(clip,progress){const mobile=window.matchMedia('(max-width:860px)').matches,source=clip&&(mobile?clip.mobile:clip.desktop)||{},x=Number(source.x)|| (mobile?72:82),y=Number(source.y)|| (mobile?66:72),endX=Number(source.endX);const endY=Number(source.endY),moving=clip&&clip.action==='flying',amount=moving?progress:0;return {x:x+((Number.isFinite(endX)?endX:x)-x)*amount,y:y+((Number.isFinite(endY)?endY:y)-y)*amount,size:Number(source.size)||(mobile?22:14),tilt:Number(source.tilt)||0}}
function renderRoamingGuide(stop,time){const clips=stop.guideClips||[],clip=clips.find(function(item){return time>=Number(item.start||0)&&time<Number(item.start||0)+Number(item.duration||0)}),action=roamingState.paused?'idle':clip&&clip.action||'idle',asset=chooseGuideAsset(action);if(!asset){roamingGuide.classList.remove('show');return}const duration=Math.max(.01,Number(clip&&clip.duration)||1),progress=Math.max(0,Math.min(1,(time-Number(clip&&clip.start||0))/duration)),placement=guidePlacement(clip,progress),key=(clip&&clip.id||'idle')+'|'+asset.url;if(roamingState.guideKey!==key){roamingState.guideKey=key;roamingGuide.src='';roamingGuide.src=asset.url}roamingGuide.style.left=placement.x+'%';roamingGuide.style.top=placement.y+'%';roamingGuide.style.width=placement.size+'%';roamingGuide.style.transform='translate(-50%,-50%) rotate('+placement.tilt+'deg)';roamingGuide.classList.add('show')}
