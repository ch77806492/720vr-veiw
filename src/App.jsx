...
function applyRoamingFrame(){const route=roamingState.route,stop=route&&route.stops[roamingState.stopIndex];if(!stop)return;const view=interpolateRoamingCamera(stop.cameraKeyframes,roamingState.elapsed);if(view&&!roamingState.paused){targetLon=Number(view.yaw)||0;targetLat=Math.max(-78,Math.min(78,Number(view.pitch)||0));camera.fov=Math.max(30,Math.min(88,Number(view.fov)||70));camera.updateProjectionMatrix()}renderRoamingGuide(stop,roamingState.elapsed);renderRoamingImage(stop,roamingState.elapsed);syncRoamingNarration(stop,roamingState.elapsed);updateRoamingControls()}
function goToRoamingStop(index){const route=roamingState.route;if(!route||!route.stops[index])return;roamingState.stopIndex=index;roamingState.elapsed=0;roamingState.lastFrame=performance.now();const stop=route.stops[index],next=project.scenes.find(function(scene){return scene.id===stop.sceneId});if(next&&next.id!==activeScene.id&&!isTransitioning)transitionToScene(next);applyRoamingFrame()}
