...
const mapRoamingAssetUrls = (roaming, mapAsset) => {
  if (!roaming) return roaming;
  const guideAssets = roaming.guide?.assets || {};
  return {
    ...roaming,
    routes: (roaming.routes || []).map((route) => ({
      ...route,
      stops: (route.stops || []).map((stop) => ({
        ...stop,
        narrationClips: (stop.narrationClips || []).map((clip) => ({
          ...clip,
          url: mapAsset(clip?.url, 'roaming-narration', clip?.assetFileName),
        })),
        imageClips: (stop.imageClips || []).map((clip) => ({
          ...clip,
          url: mapAsset(clip?.url, 'roaming-images', clip?.assetFileName),
        })),
      })),
    })),
    music: {
      ...(roaming.music || {}),
      url: mapAsset(roaming.music?.url, 'roaming-audio', roaming.music?.assetFileName),
    },
    guide: {
      ...(roaming.guide || {}),
      assets: Object.fromEntries(
        Object.entries(guideAssets).map(([action, asset]) => [
          action,
          {
            ...asset,
            url: mapAsset(asset?.url, 'roaming-guides', asset?.assetFileName),
          },
        ]),
      ),
    },
...
const stripHeavyProjectData = (project) => ({
  ...project,
  roaming: mapRoamingAssetUrls(project?.roaming, (url, kind, fileName) => pathOnlyAssetUrl(url, kind, fileName, '')),
...
const preparePublishedProjectForEditor = (project) => ({
  ...project,
  roaming: mapRoamingAssetUrls(project?.roaming, (url) => toEditorAssetUrl(url)),
  scenes: (project?.scenes || []).map((scene) => ({
    ...scene,
...
function App() {
  ...
  const uploadRoamingTrackAsset = async (file, kind, statusText) => {
    if (!file) return null;
    setIsPublishing(true);
    try {
      const uploaded = await uploadAssetFile(file, kind, file.name);
      setPublishState(statusText);
      return { name: file.name, url: uploaded.url, assetFileName: uploaded.filename || file.name };
    } catch (error) {
      console.error(error);
      setPublishState(error instanceof Error ? error.message : '漫游素材上传失败');
      return null;
    } finally {
      setIsPublishing(false);
    }
  ...
  const uploadRoamingNarration = (file) => uploadRoamingTrackAsset(file, 'roaming-narration', '解说音频已添加');
  const uploadRoamingImage = (file) => uploadRoamingTrackAsset(file, 'roaming-images', '漫游图片已添加');
  ...
  if (editorMode === 'roaming') {
    return (
      <RoamingEditor
        ...
        onUploadNarration={uploadRoamingNarration}
        onUploadImage={uploadRoamingImage}
...
function createStandalonePackage(project) {
  ...
  projectForFiles.roaming = mapRoamingAssetUrls(projectForFiles.roaming, (url, kind, fileName) =>
    toPackageAssetUrl(pathOnlyAssetUrl(url, kind, fileName, '')),
  );
  ...
  projectForFiles.roaming = {
    ...projectForFiles.roaming,
    enabled: false,
    secretEntry: {
      enabled: true,
      sceneId: openingScene?.id || 'outdoor-gate',
      yaw: 101.32,
      pitch: 16,
      width: 14,
      height: 34,
  ...
  const js = (inlineHtml.match(/<script type="module">([\s\S]*?)<\/script>/)?.[1]?.trim() || '')
    .replace(
      ...
      'const roamingPage=false,roamingState={active:false};function pauseRoaming(){}function tickRoaming(){}',
    );
...
<style>
.video-modal{z-index:44!important}.video-modal button{z-index:2!important}
...
.tour-marker.video.video-small{width:auto;height:auto;min-width:68px;min-height:30px;padding:6px 11px 6px 27px;overflow:visible;border:1px solid rgba(255,255,255,.42);border-radius:999px;background:#155ca8;color:#fff;font-size:11px;line-height:1.2;box-shadow:0 8px 20px rgba(11,63,138,.3);backdrop-filter:none}.tour-marker.video.video-small:after{display:none}.tour-marker.video.video-small:before{left:11px;top:50%;border-top-width:5px;border-bottom-width:5px;border-left-width:8px;border-left-color:#fff;transform:translateY(-50%);filter:none}.tour-marker.video .video-marker-label{position:relative;z-index:3}.tour-marker.video.video-large .video-marker-label{position:absolute;left:10px;right:10px;bottom:9px;display:block;overflow:hidden;color:#0b3f8a;font-size:24px;font-weight:400;line-height:1.2;text-align:center;text-overflow:ellipsis;text-shadow:0 1px 2px rgba(255,255,255,.72);white-space:nowrap}.tour-marker.video.video-large .video-marker-label:before{content:"";display:inline-block;margin-right:8px;width:0;height:0;border-top:7px solid transparent;border-bottom:7px solid transparent;border-left:11px solid currentColor;filter:drop-shadow(0 1px 2px rgba(255,255,255,.72));transform:translateY(1px)}.tour-marker.video.video-large:before{top:38%}
...
.roaming-control{position:fixed;left:22px;bottom:118px;z-index:17;display:none;min-height:40px;padding:0 15px;border:1px solid rgba(255,255,255,.5);border-radius:999px;background:#155ca8;color:#fff;font-size:13px;font-weight:900;box-shadow:0 14px 36px rgba(0,0,0,.28);cursor:pointer}.roaming-control.show{display:inline-flex;align-items:center}.roaming-active .roaming-control{display:none}.roaming-player{position:fixed;right:22px;top:70px;z-index:38;display:none;width:min(330px,calc(100vw - 44px));padding:12px;border:1px solid rgba(255,255,255,.28);border-radius:8px;background:rgba(5,18,37,.84);box-shadow:0 20px 56px rgba(0,0,0,.38);backdrop-filter:blur(18px)}.roaming-player.open{display:grid;gap:10px}.roaming-player-head{display:flex;align-items:center;justify-content:space-between;gap:8px}.roaming-player-head strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.roaming-player select{min-width:0;height:34px;border:1px solid rgba(255,255,255,.25);border-radius:5px;background:#17395f;color:#fff}.roaming-progress{height:5px;overflow:hidden;border-radius:99px;background:rgba(255,255,255,.18)}.roaming-progress span{display:block;width:100%;height:100%;background:#54a6f5;transform:scaleX(0);transform-origin:left center}.roaming-status{display:flex;justify-content:space-between;color:rgba(255,255,255,.76);font-size:11px}.roaming-actions{display:grid;grid-template-columns:34px 1fr 34px 34px;gap:6px}.roaming-actions button{height:34px;border:1px solid rgba(255,255,255,.22);border-radius:5px;background:rgba(255,255,255,.1);color:#fff;font-weight:800;cursor:pointer}.roaming-actions button:nth-child(2){background:#2379cf}.roaming-guide-overlay{position:fixed;z-index:26;display:none;height:auto;max-height:44vh;object-fit:contain;pointer-events:none;transform-origin:center;filter:drop-shadow(0 12px 16px rgba(0,0,0,.22));will-change:left,top,transform}.roaming-guide-overlay.show{display:block}.roaming-image-overlay{position:fixed;z-index:27;display:none;height:auto;max-height:76vh;object-fit:contain;pointer-events:none;transform:translate(-50%,-50%);filter:drop-shadow(0 14px 24px rgba(0,0,0,.28))}.roaming-image-overlay.show{display:block}@media(max-width:860px){.roaming-control{left:0;bottom:auto;top:calc(50% + 52px);min-height:44px;padding:0 10px;border-radius:0 999px 999px 0;transform:translateY(-50%)}.roaming-player{left:12px;right:12px;top:auto;bottom:12px;width:auto}.roaming-guide-overlay{max-height:36vh}.roaming-image-overlay{max-height:58vh}.roaming-active .bottom-dock{opacity:0;pointer-events:none}}
.roaming-entry-dialog{position:fixed;inset:0;z-index:48;display:none;place-items:center;padding:20px;background:rgba(2,8,23,.48);backdrop-filter:blur(8px)}.roaming-entry-dialog.open{display:grid}.roaming-entry-panel{width:min(360px,calc(100vw - 32px));padding:22px;border:1px solid rgba(255,255,255,.3);border-radius:8px;background:rgba(8,27,52,.94);box-shadow:0 24px 70px rgba(0,0,0,.42);color:#fff}.roaming-entry-panel h2{margin:0;font-size:20px}.roaming-entry-panel p{margin:9px 0 20px;color:rgba(255,255,255,.72);font-size:13px}.roaming-entry-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.roaming-entry-actions button{height:40px;border:1px solid rgba(255,255,255,.24);border-radius:6px;background:rgba(255,255,255,.08);color:#fff;font-weight:900;cursor:pointer}.roaming-entry-actions .primary{border-color:#54a6f5;background:#2379cf}
</style>
...
<button class="roaming-control" id="roamingControl" type="button">开始漫游</button>
<section class="roaming-player" id="roamingPlayer" aria-label="漫游控制">
  <div class="roaming-player-head"><strong id="roamingRouteName">校园漫游</strong><select id="roamingRouteSelect" aria-label="漫游路线"></select></div>
...
<audio id="roamingMusic" preload="auto" loop playsinline></audio>
<audio id="roamingNarration" preload="auto" playsinline></audio>
...
function setScene(id){if(roamingState.active)pauseRoaming('已暂停，可自由查看当前场景');const next=project.scenes.find(function(s){return s.id===id});if(!next||next.id===activeScene.id||isTransitioning)return;transitionToScene(next)}
...
function loadKrpanoTiles(scene){const tiles=scene.krpanoTiles;return new Promise(function(resolve){const nextGroup=new THREE.Group(),nextMaterials=[],faces=['f','r','b','l','u','d'],grid=Number(tiles.grid)||2,tasks=[];faces.forEach(function(face){for(let row=1;row<=grid;row+=1){for(let col=1;col<=grid;col+=1){tasks.push(textureFromUrl(tilePatternUrl(tiles,face,row,col)).then(function(texture){if(!texture)return;const mat=new THREE.MeshBasicMaterial({map:texture,transparent:true,opacity:1,side:THREE.DoubleSide});nextMaterials.push(mat);nextGroup.add(makeTileMesh(face,row,col,grid,mat))}))}}});Promise.all(tasks).then(function(){if(!nextMaterials.length){resolve(false);return}disposeTileGroup();while(nextGroup.children.length)tileGroup.add(nextGroup.children[0]);tileMaterials=nextMaterials;introImage=null;sphere.visible=false;tileGroup.rotation.y=THREE.MathUtils.degToRad(sceneRoll(scene));tileGroup.visible=true;resolve(true)})})}
...
async function createAnimatedTexture(url,playCount,onReady){
  const canvas=document.createElement('canvas'),ctx=canvas.getContext('2d',{alpha:true}),texture=new THREE.CanvasTexture(canvas);canvas.width=2;canvas.height=2;texture.colorSpace=THREE.SRGBColorSpace;texture.minFilter=THREE.LinearFilter;texture.magFilter=THREE.LinearFilter;texture.generateMipmaps=false;let disposed=false,timer=0,raf=0,decoder=null,objectUrl='',runId=0,play=function(){};
  function resize(w,h){w=Math.max(1,Number(w)||1);h=Math.max(1,Number(h)||1);if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h}onReady&&onReady(w/h)}
  function drawFrame(frame){resize(frame.displayWidth||frame.codedWidth,frame.displayHeight||frame.codedHeight);ctx.clearRect(0,0,canvas.width,canvas.height);ctx.drawImage(frame,0,0,canvas.width,canvas.height);texture.needsUpdate=true}
  try{const response=await fetch(url,{cache:'no-store'});if(!response.ok)throw new Error('动态图片加载失败');const blob=await response.blob();if('ImageDecoder' in window){const fallback=String(url).toLowerCase().includes('.webp')?'image/webp':'image/gif';decoder=new window.ImageDecoder({data:await blob.arrayBuffer(),type:blob.type||fallback});await decoder.tracks.ready;const track=decoder.tracks.selectedTrack,frames=Math.max(1,Number(track&&track.frameCount)||1),first=await decoder.decode({frameIndex:0});drawFrame(first.image);first.image.close();play=function(times){const token=++runId,infinite=Number(times)===0,loops=infinite?1:Math.max(1,Math.min(10,Number(times)||1));clearTimeout(timer);(async function(){for(let loop=0;infinite||loop<loops;loop+=1){for(let index=0;index<frames;index+=1){if(disposed||token!==runId)return;try{const result=await decoder.decode({frameIndex:index}),frame=result.image;if(disposed||token!==runId){frame.close();return}drawFrame(frame);const duration=Math.max(20,Number(frame.duration||100000)/1000);frame.close();if(infinite||loop<loops-1||index<frames-1)await new Promise(function(resolve){timer=setTimeout(resolve,duration)})}catch(error){if(!disposed&&error&&error.name!=='AbortError')console.warn(error);return}}}})()} }else{const image=new Image();function loadImage(){return new Promise(function(resolve,reject){image.onload=resolve;image.onerror=reject})}objectUrl=URL.createObjectURL(blob);image.src=objectUrl;await loadImage();resize(image.naturalWidth,image.naturalHeight);ctx.clearRect(0,0,canvas.width,canvas.height);ctx.drawImage(image,0,0,canvas.width,canvas.height);texture.needsUpdate=true;play=function(times){const token=++runId,infinite=Number(times)===0,runDuration=infinite?Infinity:2000*Math.max(1,Math.min(10,Number(times)||1));cancelAnimationFrame(raf);if(objectUrl)URL.revokeObjectURL(objectUrl);objectUrl=URL.createObjectURL(blob);image.src=objectUrl;image.onload=function(){const started=performance.now();function draw(){if(disposed||token!==runId)return;ctx.clearRect(0,0,canvas.width,canvas.height);ctx.drawImage(image,0,0,canvas.width,canvas.height);texture.needsUpdate=true;if(infinite||performance.now()-started<runDuration)raf=requestAnimationFrame(draw)}draw()}}}}catch(error){if(!disposed&&error&&error.name!=='AbortError')console.warn(error)}const configuredLoops=Math.max(0,Math.min(10,Number(playCount)||0));return{texture:texture,playInitial:function(){play(configuredLoops)},playOnce:function(){play(configuredLoops===0?0:1)},dispose:function(){disposed=true;runId+=1;clearTimeout(timer);cancelAnimationFrame(raf);decoder&&decoder.close&&decoder.close();if(objectUrl)URL.revokeObjectURL(objectUrl);objectUrl='';texture.dispose()}}}
...
function renderAnimatedSpots(group,scene,setControllers,oldControllers){clearAnimatedSpots(group,oldControllers);const controllers=[];setControllers(controllers);(scene.animatedSpots||[]).filter(function(spot){return spot.imageUrl}).forEach(function(spot){const material=new THREE.MeshBasicMaterial({transparent:true,opacity:0,side:THREE.DoubleSide,depthTest:false,depthWrite:false}),mesh=new THREE.Mesh(new THREE.PlaneGeometry(1,1),material);mesh.renderOrder=12;mesh.userData.animatedSpot=spot;mesh.userData.animatedController={playOnce:function(){mesh.userData.pendingPlay=true}};placeAnimatedPlane(mesh,spot,1);group.add(mesh);createAnimatedTexture(spot.imageUrl,spot.playCount,function(aspect){placeAnimatedPlane(mesh,spot,aspect)}).then(function(controller){if(!group.children.includes(mesh)){controller.dispose();return}controllers.push(controller);mesh.userData.animatedController=controller;material.map=controller.texture;material.opacity=1;material.needsUpdate=true;controller.playInitial();if(mesh.userData.pendingPlay){mesh.userData.pendingPlay=false;controller.playOnce()}})})}
const animatedSpotPointer=new THREE.Vector2(),animatedSpotRaycaster=new THREE.Raycaster();
function playAnimatedSpotAt(clientX,clientY){
  if(isIntro||isTransitioning)return false;
  camera.updateMatrixWorld();animatedGroup.updateMatrixWorld(true);
  animatedSpotPointer.set(clientX/window.innerWidth*2-1,-clientY/window.innerHeight*2+1);animatedSpotRaycaster.setFromCamera(animatedSpotPointer,camera);
  const rayHit=animatedSpotRaycaster.intersectObjects(animatedGroup.children,false)[0],fallbackHit=animatedGroup.children.find(function(mesh){const center=mesh.position.clone().project(camera);if(center.z<0||center.z>1)return false;const corners=[[-.5,-.5],[.5,-.5],[.5,.5],[-.5,.5]].map(function(corner){const point=new THREE.Vector3(corner[0],corner[1],0).applyMatrix4(mesh.matrixWorld).project(camera);return {x:(point.x*.5+.5)*window.innerWidth,y:(-point.y*.5+.5)*window.innerHeight}}),xs=corners.map(function(point){return point.x}),ys=corners.map(function(point){return point.y}),padding=18;return clientX>=Math.min.apply(null,xs)-padding&&clientX<=Math.max.apply(null,xs)+padding&&clientY>=Math.min.apply(null,ys)-padding&&clientY<=Math.max.apply(null,ys)+padding}),mesh=rayHit&&rayHit.object||fallbackHit,controller=mesh&&mesh.userData.animatedController;
  if(!controller||!controller.playOnce)return false;
  controller.playOnce();stage.dataset.animatedSpotPlays=String((Number(stage.dataset.animatedSpotPlays)||0)+1);return true;
}
...
function elapsedBeforeStop(route,index){return (route&&route.stops||[]).slice(0,index).reduce(function(total,stop){return total+(Number(stop.duration)||0)},0)}
function updateRoamingControls(message){const route=roamingState.route,total=routeDuration(route),current=elapsedBeforeStop(route,roamingState.stopIndex)+roamingState.elapsed,stop=route&&route.stops[roamingState.stopIndex];roamingRouteName.textContent=route?route.name:'校园漫游';roamingStatus.textContent=message||(roamingState.paused?'漫游已暂停':stop?'正在浏览 '+(project.scenes.find(function(scene){return scene.id===stop.sceneId})||{}).name:'准备开始');roamingTime.textContent=formatRoamingTime(current)+' / '+formatRoamingTime(total);roamingProgress.style.transform='scaleX('+(total?Math.min(1,current/total):0).toFixed(4)+')';roamingPause.textContent=roamingState.paused?'继续漫游':'暂停漫游';roamingPrev.disabled=!route||roamingState.stopIndex<=0;roamingNext.disabled=!route||roamingState.stopIndex>=(route.stops.length-1)}
function resetRoamingOverlays(){roamingGuide.classList.remove('show');roamingGuide.removeAttribute('src');roamingImage.classList.remove('show');roamingImage.removeAttribute('src');roamingNarration.pause();roamingNarration.removeAttribute('src');roamingState.guideKey='';roamingState.narrationKey=''}
function guideAction(action){return {greeting:'welcome',talking:'selfIntroduction',flying:'flyingTransition',idle:'hoverExplanation',leaving:'flyingTransition',pointing:'pointRight'}[action]||action||'hoverExplanation'}
function chooseGuideAsset(action){const assets=roamingConfig.guide&&roamingConfig.guide.assets||{},preferred=assets[guideAction(action)],hover=assets.hoverExplanation||assets.idle;if(preferred&&preferred.url)return preferred;if(hover&&hover.url)return hover;return Object.values(assets).find(function(asset){return asset&&asset.url})||null}
function guidePlacement(clip,progress){const mobile=window.matchMedia('(max-width:860px)').matches,source=clip&&(mobile?clip.mobile:clip.desktop)||{},x=Number(source.x)|| (mobile?72:82),y=Number(source.y)|| (mobile?66:72),endX=Number(source.endX);const endY=Number(source.endY),moving=clip&&guideAction(clip.action)==='flyingTransition',amount=moving?progress:0;return {x:x+((Number.isFinite(endX)?endX:x)-x)*amount,y:y+((Number.isFinite(endY)?endY:y)-y)*amount,size:Number(source.size)||(mobile?22:14),tilt:Number(source.tilt)||0}}
function renderRoamingGuide(stop,time){const clips=stop.guideClips||[],clip=clips.find(function(item){return time>=Number(item.start||0)&&time<Number(item.start||0)+Number(item.duration||0)}),action=roamingState.paused?'hoverExplanation':guideAction(clip&&clip.action),asset=chooseGuideAsset(action);if(!asset){roamingGuide.classList.remove('show');return}const duration=Math.max(.01,Number(clip&&clip.duration)||1),progress=Math.max(0,Math.min(1,(time-Number(clip&&clip.start||0))/duration)),placement=guidePlacement(clip,progress),key=(clip&&clip.id||'hoverExplanation')+'|'+asset.url;if(roamingState.guideKey!==key){roamingState.guideKey=key;roamingGuide.src='';roamingGuide.src=asset.url}roamingGuide.style.left=placement.x+'%';roamingGuide.style.top=placement.y+'%';roamingGuide.style.width=placement.size+'%';roamingGuide.style.transform='translate(-50%,-50%) rotate('+placement.tilt+'deg)';roamingGuide.classList.add('show')}
function renderRoamingImage(stop,time){const clip=(stop.imageClips||[]).find(function(item){return time>=Number(item.start||0)&&time<Number(item.start||0)+Number(item.duration||0)});if(!clip||!clip.url){roamingImage.classList.remove('show');return}const mobile=window.matchMedia('(max-width:860px)').matches,placement=mobile?clip.mobile||{}:clip.desktop||{};if(roamingImage.getAttribute('src')!==clip.url)roamingImage.src=clip.url;roamingImage.style.left=(Number(placement.x)||50)+'%';roamingImage.style.top=(Number(placement.y)||50)+'%';roamingImage.style.width=(Number(placement.size)||(mobile?60:28))+'%';roamingImage.classList.add('show')}
function syncRoamingNarration(stop,time){const clip=(stop.narrationClips||[]).find(function(item){return time>=Number(item.start||0)&&time<Number(item.start||0)+Number(item.duration||0)});if(!clip||!clip.url||roamingState.paused||isTransitioning||isIntro){roamingNarration.pause();return}const key=clip.id+'|'+clip.url,expected=Math.max(0,Number(clip.trimStart||0)+time-Number(clip.start||0));if(roamingState.narrationKey!==key){roamingState.narrationKey=key;roamingNarration.src=clip.url;roamingNarration.currentTime=expected;const playResult=roamingNarration.play();if(playResult&&playResult.catch)playResult.catch(function(error){console.warn(error)})}else if(Math.abs(roamingNarration.currentTime-expected)>.8){roamingNarration.currentTime=expected}if(roamingNarration.paused){const playResult=roamingNarration.play();if(playResult&&playResult.catch)playResult.catch(function(error){console.warn(error)})}}
function interpolateRoamingCamera(keyframes,time){const frames=(keyframes||[]).slice().sort(function(a,b){return Number(a.time)-Number(b.time)});if(!frames.length)return null;if(frames.length===1||time<=Number(frames[0].time))return frames[0];const last=frames[frames.length-1];if(time>=Number(last.time))return last;for(let index=0;index<frames.length-1;index+=1){const from=frames[index],to=frames[index+1],start=Number(from.time),end=Number(to.time);if(time<=end){const progress=(time-start)/Math.max(.001,end-start),yaw=Number(from.yaw)+normalizeAngle(Number(to.yaw)-Number(from.yaw))*progress;return {yaw:yaw,pitch:Number(from.pitch)+(Number(to.pitch)-Number(from.pitch))*progress,fov:Number(from.fov||70)+(Number(to.fov||70)-Number(from.fov||70))*progress}}}return last}
...
if('DeviceOrientationEvent' in window)gyroControl.classList.add('available');gyroControl.onclick=function(e){e.stopPropagation();if(roamingState.active)pauseRoaming('已暂停，可使用陀螺仪查看');toggleGyroscope()};
musicControl.onclick=function(e){e.stopPropagation();toggleBackgroundMusic()};
let activeTouchId=null,mainPressX=0,mainPressY=0,mainPressMoved=false;
function prepareMainDrag(x,y,touch){if(roamingState.active)pauseRoaming('已暂停，可自由查看当前场景');dragging=true;isTouchDragging=touch;mainPressX=x;mainPressY=y;mainPressMoved=false;dragFactorX=touch?Math.max(.42,180/Math.max(window.innerWidth,320)):.12;dragFactorY=touch?Math.max(.12,90/Math.max(window.innerHeight,480)):.12;stage.dataset.dragSensitivity=dragFactorX.toFixed(3);stage.dataset.dragInput=touch?'touch':'pointer';startX=x;startY=y;document.body.classList.remove('quick-open','map-open')}
function applyMainDrag(x,y){const dx=x-startX,dy=y-startY;if(Math.hypot(x-mainPressX,y-mainPressY)>8)mainPressMoved=true;targetLon+=dx*dragFactorX;targetLat=THREE.MathUtils.clamp(targetLat+dy*dragFactorY,-78,78);if(isTouchDragging){lon=targetLon;lat=targetLat}startX=x;startY=y}
...
stage.addEventListener('pointerup',finishDrag);stage.addEventListener('pointercancel',finishDrag);stage.addEventListener('click',function(e){if(!mainPressMoved)playAnimatedSpotAt(e.clientX,e.clientY)});
...
window.addEventListener('touchend',function(e){if(activeTouchId==null)return;const touch=Array.from(e.changedTouches).find(function(item){return item.identifier===activeTouchId});if(touch){if(!mainPressMoved)playAnimatedSpotAt(touch.clientX,touch.clientY);finishDrag()}},{passive:true});
window.addEventListener('touchcancel',finishDrag,{passive:true});
stage.addEventListener('wheel',function(e){e.preventDefault();if(roamingState.active)pauseRoaming('已暂停，可自由查看当前场景');camera.fov=THREE.MathUtils.clamp(camera.fov+e.deltaY*.03,30,88);camera.updateProjectionMatrix()},{passive:false});
