...
async function createAnimatedTexture(url,playCount,onReady){
  ...
  try{const response=await fetch(url,{cache:'no-store'});if(!response.ok)throw new Error('动态图片加载失败');const blob=await response.blob();if('ImageDecoder' in window){const fallback=String(url).toLowerCase().includes('.webp')?'image/webp':'image/gif';decoder=new window.ImageDecoder({data:await blob.arrayBuffer(),type:blob.type||fallback});await decoder.tracks.ready;const track=decoder.tracks.selectedTrack,frames=Math.max(1,Number(track&&track.frameCount)||1),first=await decoder.decode({frameIndex:0});drawFrame(first.image);first.image.close();playOnce=function(){const token=++runId;clearTimeout(timer);(async function(){for(let index=0;index<frames;index+=1){if(disposed||token!==runId)return;try{const result=await decoder.decode({frameIndex:index}),frame=result.image;if(disposed||token!==runId){frame.close();return}drawFrame(frame);const duration=Math.max(20,Number(frame.duration||100000)/1000);frame.close();if(index<frames-1)await new Promise(function(resolve){timer=setTimeout(resolve,duration)})}catch(error){if(!disposed&&error&&error.name!=='AbortError')console.warn(error);return}}})()} }else{const image=new Image();function loadImage(){return new Promise(function(resolve,reject){image.onload=resolve;image.onerror=reject})}objectUrl=URL.createObjectURL(blob);image.src=objectUrl;await loadImage();resize(image.naturalWidth,image.naturalHeight);ctx.clearRect(0,0,canvas.width,canvas.height);ctx.drawImage(image,0,0,canvas.width,canvas.height);texture.needsUpdate=true;playOnce=function(){const token=++runId;cancelAnimationFrame(raf);if(objectUrl)URL.revokeObjectURL(objectUrl);objectUrl=URL.createObjectURL(blob);image.src=objectUrl;image.onload=function(){const started=performance.now();function draw(){if(disposed||token!==runId)return;ctx.clearRect(0,0,canvas.width,canvas.height);ctx.drawImage(image,0,0,canvas.width,canvas.height);texture.needsUpdate=true;if(performance.now()-started<2000)raf=requestAnimationFrame(draw)}draw()}}}}catch(error){if(!disposed&&error&&error.name!=='AbortError')console.warn(error)}return{texture:texture,playOnce:playOnce,dispose:function(){disposed=true;runId+=1;clearTimeout(timer);cancelAnimationFrame(raf);decoder&&decoder.close&&decoder.close();if(objectUrl)URL.revokeObjectURL(objectUrl);objectUrl='';texture.dispose()}}}
