...
<div class="roaming-text-overlay" id="roamingText"></div>
...
const roamingConfig=project.roaming||{},roamingRoutes=(roamingConfig.routes||[]).filter(function(route){return (route.stops||[]).length}),roamingControl=document.getElementById('roamingControl'),roamingPlayer=document.getElementById('roamingPlayer'),roamingRouteName=document.getElementById('roamingRouteName'),roamingRouteSelect=document.getElementById('roamingRouteSelect'),roamingProgress=document.getElementById('roamingProgress'),roamingStatus=document.getElementById('roamingStatus'),roamingTime=document.getElementById('roamingTime'),roamingPause=document.getElementById('roamingPause'),roamingPrev=document.getElementById('roamingPrev'),roamingNext=document.getElementById('roamingNext'),roamingExit=document.getElementById('roamingExit'),roamingGuide=document.getElementById('roamingGuide'),roamingText=document.getElementById('roamingText'),roamingMusic=document.getElementById('roamingMusic');
const roamingState={active:false,paused:false,route:null,stopIndex:0,elapsed:0,lastFrame:0,resumeBackground:false,guideKey:''};
...
function resetRoamingOverlays(){roamingGuide.classList.remove('show');roamingGuide.removeAttribute('src');roamingText.className='roaming-text-overlay';roamingText.textContent='';roamingState.guideKey=''}
...
function renderRoamingText(stop,time){const clip=(stop.textClips||[]).find(function(item){return time>=Number(item.start||0)&&time<Number(item.start||0)+Number(item.duration||0)});if(!clip){roamingText.className='roaming-text-overlay';roamingText.textContent='';return}roamingText.className='roaming-text-overlay show theme-'+(clip.theme||'dark');roamingText.textContent=clip.text||'';roamingText.style.fontSize=(Number(clip.fontSize)||22)+'px';roamingText.style.top=clip.position==='top'?'8%':clip.position==='center'?'50%':'auto';roamingText.style.bottom=clip.position==='bottom'?'10%':'auto';roamingText.style.transform=clip.position==='center'?'translate(-50%,-50%)':'translateX(-50%)'}
...
function pauseRoaming(message){if(!roamingState.active||roamingState.paused)return;roamingState.paused=true;roamingMusic.pause();roamingControl.textContent='继续漫游';applyRoamingFrame();updateRoamingControls(message||'漫游已暂停')}
...
stage.addEventListener('wheel',function(e){e.preventDefault();if(roamingState.active)pauseRoaming('已暂停，可自由查看当前场景');camera.fov=THREE.MathUtils.clamp(camera.fov+e.deltaY*.03,38,88);camera.updateProjectionMatrix()},{passive:false});
