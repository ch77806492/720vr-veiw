...
const secretEntryPointer=new THREE.Vector2(),secretEntryRaycaster=new THREE.Raycaster();let secretEntryClickCount=0,secretEntryLastClick=0;
...
function trySecretRoamingEntry(clientX,clientY){const entry=roamingConfig.secretEntry||{};if(!entry.enabled||!roamingRoutes.length||roamingState.active||isIntro||isTransitioning||activeScene.id!==entry.sceneId)return;secretEntryPointer.set(clientX/window.innerWidth*2-1,-clientY/window.innerHeight*2+1);secretEntryRaycaster.setFromCamera(secretEntryPointer,camera);const direction=secretEntryRaycaster.ray.direction,yaw=THREE.MathUtils.radToDeg(Math.atan2(direction.z,direction.x)),pitch=THREE.MathUtils.radToDeg(Math.asin(direction.y)),inside=Math.abs(normalizeAngle(yaw-Number(entry.yaw||0)))<=Number(entry.width||14)/2&&Math.abs(pitch-Number(entry.pitch||0))<=Number(entry.height||34)/2;if(!inside){secretEntryClickCount=0;stage.dataset.secretEntryClicks='0';return}const now=Date.now();secretEntryClickCount=now-secretEntryLastClick>4000?1:secretEntryClickCount+1;secretEntryLastClick=now;stage.dataset.secretEntryClicks=String(secretEntryClickCount);if(secretEntryClickCount>=Math.max(2,Number(entry.clicks)||5)){secretEntryClickCount=0;stage.dataset.secretEntryClicks='0';setRoamingEntryDialog(true)}}
...
roamingEntryCancel.onclick=function(){setRoamingEntryDialog(false)};roamingEntryConfirm.onclick=function(){setRoamingEntryDialog(false);startRoaming(roamingRouteSelect.value)};roamingEntryDialog.onclick=function(e){if(e.target===roamingEntryDialog)setRoamingEntryDialog(false)};window.addEventListener('keydown',function(e){if(e.key==='Escape')setRoamingEntryDialog(false)});
...
stage.addEventListener('pointerup',function(e){if(!mainPressMoved)trySecretRoamingEntry(e.clientX,e.clientY);finishDrag()});stage.addEventListener('pointercancel',finishDrag);
...
window.addEventListener('touchend',function(e){if(activeTouchId==null)return;const touch=Array.from(e.changedTouches).find(function(item){return item.identifier===activeTouchId});if(touch){if(!mainPressMoved)trySecretRoamingEntry(touch.clientX,touch.clientY);finishDrag()}},{passive:true});
